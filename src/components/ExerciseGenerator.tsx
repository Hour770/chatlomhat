...
export default function ExerciseGenerator() {
  ...
  const { isAuthenticated, addToExerciseHistory } = useAuth();
  ...
  const generateExercises = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (isAuthenticated && data.exercises.length > 0) {
          const exercisesText = data.exercises.map((e: Exercise, i: number) => 
            `${i + 1}. ${e.problem}`
          ).join('\n');
          addToExerciseHistory(topic, difficulty, exercisesText);
        }
      } else {
        setResponse({ 
          exercises: [], 
